...
toMigrate(N,(SWReqs,HWDiff,TReqs)) :-
    %(dif(SWReqs,[]); HWDiff =\= 0; dif(TReqs,[])),
    node(N, SWCaps, HWCaps, TCaps), 
    hwTh(T), \+ (swReqsOK(SWReqs, SWCaps), HWCaps > HWDiff + T, thingReqsOK(TReqs, TCaps)).
toMigrate(N,_) :-
    \+ node(N, _, _, _).
...
toUpdate(N,(SWReqs,HWDiff,TReqs)) :-
    HWDiff =\= 0,
    node(N, SWCaps, HWCaps, TCaps),
