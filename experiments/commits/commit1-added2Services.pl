...
application(vrApp, [videoStorage, sceneSelector, vrDriver, tokensDealer, geoLocalisator]).
...
service(geoLocalisator, [gcc, make], 2, []).
...
s2s(sceneSelector, geoLocalisator, 50, 2).
s2s(geoLocalisator, tokensDealer, 200, 0.5).
s2s(tokensDealer, geoLocalisator, 200, 1).
